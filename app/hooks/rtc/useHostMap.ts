...
const getBrowserInfo = () => {
  ...
  return {
    ...
    isSafari,
    isEdge,
    browserName: isFirefox ? 'Firefox' : isChrome ? 'Chrome' : isSafari ? 'Safari' : isEdge ? 'Edge' : 'Unknown'
  };
};
...
export function useHostPeerSession({
  ...
  useEffect(() => {
    ...
    p.on('connection', (conn) => {
      console.log('[useHostMap] 🔗 New connection attempt from peer:', conn.peer);
      ...
      conn.on('open', () => {
        console.log('[useHostMap] ✅ Connection fully established with:', conn.peer);
        ...
        console.log('[useHostMap] Game state snapshot:', snap ? 'Available' : 'Not available');
        
        if (snap) {
          setTimeout(() => {
            try {
              conn.send({ type: 'snapshot', snapShot: snap } as SnapshotUpdate);
              console.log('[useHostMap] ✅ Game state sent successfully to', conn.peer);
            } catch (error) {
              console.error('[useHostMap] ❌ Failed to send game state to', conn.peer, error);
            }
          }, 1000);
        }
      });
        ...
        if (data && typeof data === 'object' && 'type' in data) {
          switch (data.type) {
            case 'moveCharacter': {
              ...
              console.log('[useHostMap] Processing move character:', { characterId, x, y });
              moveCharacterCallback?.(characterId, x, y);
              break;
            }
            default:
              console.warn(
                `[useHostMap] ⚠️ Unknown data type received from ${conn.peer}:`, data
              );
          }
        } else {
          console.warn('[useHostMap] ⚠️ Invalid data format received from', conn.peer, ':', data);
        }
      });

      // this is what happens when a peer disconnects nicely
      conn.on('close', () => {
        console.log('[useHostMap] 🔌 Connection closed gracefully with:', conn.peer);
        setConnections((prev) => {
      ...
      conn.on('iceStateChanged', () => {
        ...
        if (connectionState === 'failed' || connectionState === 'disconnected' || iceState === 'failed') {
          console.log('[useHostMap] 💥 Connection failed/disconnected with:', conn.peer);
          setConnections((prev) => {
            const updated = prev.filter((c) => c.peer !== conn.peer);
            connectionsRef.current = updated;
            console.log('[useHostMap] Removed failed connection. Remaining:', updated.length);
            return updated;
          });
      ...
      conn.on('error', (err) => {
        console.error('[useHostMap] ❌ Connection error with', conn.peer, ':', err);
        ...
        setConnections((prev) => {
          const updated = prev.filter((c) => c.peer !== conn.peer);
          connectionsRef.current = updated;
          console.log('[useHostMap] Removed errored connection. Remaining:', updated.length);
          return updated;
        });
      });
    });
    ...
    p.on('disconnected', () => {
      console.warn('[useHostMap] 🔌 Host peer disconnected from signaling server');
    ...
    return () => {
      ...
      clearTimeout(openTimeout);
      if (p && !p.destroyed) {
        console.log('[useHostMap] Destroying peer with', connectionsRef.current.length, 'connections');
        p.destroy();
      }
      setConnections([]);
  ...
  const broadcastData = (data: unknown) => {
    const activeConnections = connectionsRef.current.filter(conn => conn.open);
    console.log('[useHostMap] 📡 Broadcasting data to', activeConnections.length, 'peers:', data);
    ...
    connectionsRef.current.forEach((conn) => {
      if (conn.open) {
        try {
          ...
          failCount++;
          console.error('[useHostMap] ❌ Failed to send data to', conn.peer, ':', error);
        }
      } else {
    ...
    console.log('[useHostMap] Broadcast complete:', { successCount, failCount, total: connectionsRef.current.length });
  };
  ...
  const sendDataToPeer = (peerId: string, data: unknown) => {
    ...
    if (conn?.open) {
      try {
        ...
        console.error('[useHostMap] ❌ Failed to send data to', peerId, ':', error);
      }
    } else {
      console.warn('[useHostMap] ⚠️ Cannot send to', peerId, '- connection not open or not found');
      console.log('[useHostMap] Available connections:', connectionsRef.current.map(c => ({ peer: c.peer, open: c.open })));
    }
  };
