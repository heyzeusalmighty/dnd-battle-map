...
const checkFirefoxWebRTCSupport = () => {
  ...
  if (getBrowserInfo().isFirefox) {
    ...
    if (match) {
      ...
      if (version < 38) {
        info.potentialIssues.push('Firefox version too old for reliable WebRTC support');
      }
      if (version >= 72 && version < 78) {
        info.potentialIssues.push('Known WebRTC reliability issues in Firefox 72-77');
      }
      if (version < 60) {
        info.potentialIssues.push('Firefox versions below 60 have limited WebRTC data channel support');
      }
    }

    // Check for Firefox-specific settings that might affect WebRTC
    if (typeof navigator.mediaDevices === 'undefined') {
      info.potentialIssues.push('MediaDevices not available - might indicate privacy settings blocking WebRTC');
    }
    ...
    try {
      ...
      info.potentialIssues.push('WebRTC configuration test failed - check browser settings');
    }

    // Check Firefox privacy settings
    if (navigator.doNotTrack === '1') {
      info.potentialIssues.push('Do Not Track is enabled - may affect WebRTC connections');
    }
    ...
    try {
      ...
      if (!('indexedDB' in window)) {
        info.potentialIssues.push('Private browsing mode detected - WebRTC may be limited');
      }
    } catch {
...
export function useGuestMap({
  ...
  useEffect(() => {
    ...
    if (browserInfo.isFirefox) {
      ...
      if (firefoxInfo.potentialIssues.length > 0) {
        console.warn('[useGuestMap] Potential Firefox WebRTC issues detected:', firefoxInfo.potentialIssues);
      }
    }
    ...
    const peerConfig = {
      ...
      config: {
        ...
        bundlePolicy: browserInfo.isFirefox ? 'max-bundle' as RTCBundlePolicy : 'balanced' as RTCBundlePolicy,
    ...
    peer.on('open', (id) => {
      ...
      const connectToHost = (attempt = 1) => {
        ...
        const attemptTimeout = setTimeout(() => {
          ...
          if (conn && !conn.open) {
            ...
            if (attempt < 3 && browserInfo.isFirefox) {
              console.log(`[useGuestMap] Retrying connection (attempt ${attempt + 1}/3)`);
        ...
        conn.peerConnection.addEventListener('connectionstatechange', () => {
          console.log('[useGuestMap] Connection state changed:', conn.peerConnection.connectionState);
        });

        conn.peerConnection.addEventListener('iceconnectionstatechange', () => {
          console.log('[useGuestMap] ICE connection state changed:', conn.peerConnection.iceConnectionState);
        });

        conn.peerConnection.addEventListener('icegatheringstatechange', () => {
          console.log('[useGuestMap] ICE gathering state changed:', conn.peerConnection.iceGatheringState);
        });

        conn.peerConnection.addEventListener('signalingstatechange', () => {
          console.log('[useGuestMap] Signaling state changed:', conn.peerConnection.signalingState);
        });
      }
        ...
        conn.on('error', (error) => {
          ...
          if (browserInfo.isFirefox && attempt < 3) {
            const shouldRetry = error.name === 'peer-unavailable' || 
                               error.name === 'network' ||
                               error.message.includes('connection failed') ||
                               error.message.includes('timeout');
            
            if (shouldRetry) {
              console.log(`[useGuestMap] Retrying due to Firefox error (attempt ${attempt + 1}/3)`);
  ...
  const debugInfo = {
    ...
    firefoxInfo: getBrowserInfo().isFirefox ? checkFirefoxWebRTCSupport() : null,
