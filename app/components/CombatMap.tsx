...
export function CombatMap({
  ...
  const getTokenSize = (size: string) => {
    switch (size) {
      case 'small':
        return GRID_SIZE * 0.6;
      case 'medium':
        return GRID_SIZE * 0.8;
      case 'large':
        return GRID_SIZE * 1.2;
      case 'huge':
        return GRID_SIZE * 1.6;
      default:
        return GRID_SIZE * 0.8;
    }
  };
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => {
      try {
        ...
        const x = Math.floor(
          (e.clientX - mapRect.left - dragOffset.x) / GRID_SIZE
        );
        const y = Math.floor(
          (e.clientY - mapRect.top - dragOffset.y) / GRID_SIZE
        );
  ...
  const handleTerrainRightClick = useCallback(
    (e: React.MouseEvent, terrainId: string) => {
      try {
        e.preventDefault();
          ...
          {measurements
            ...
            .map((measurement) => (
              <g key={measurement.id}>
                ...
                <text
                  x={
                    ((measurement.startX + measurement.endX) * GRID_SIZE) / 2 +
                    GRID_SIZE / 2
                  }
                  y={
                    ((measurement.startY + measurement.endY) * GRID_SIZE) / 2 +
                    GRID_SIZE / 2
                  }
                  fill="#FF6B35"
                  fontSize="12"
        ...
        {terrain
          ...
          .map((tile) => (
            <div
              ...
              title={
                selectedTool === 'select' ? 'Right-click to delete' : undefined
              }
            >
              {tile.type === 'door' && (
                  ...
                  <div className="w-8 h-1 bg-gray-700 rounded">
                    <div
                      ...
                      style={{
                        ...
                        backgroundColor:
                          safeHp / safeMaxHp > 0.5
                            ? '#10B981'
                            : safeHp / safeMaxHp > 0.25
                              ? '#F59E0B'
                              : '#EF4444',
                      }}
                    />
